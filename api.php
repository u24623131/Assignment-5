...
class API
{
    ...
    private function handleLogin($input)
    {
        ...
        $stmt = $this->DB_Connection->prepare("SELECT User_Type, Password, Salt, API_Key FROM Users WHERE Email = ?");
    ...
    private function getAllProducts()
    {
        ...
        // Query the products
        $result = $this->DB_Connection->query("SELECT Products.Product_No,
            Products.Title,
            Products.Category,
    ...
    private function handleProductByRetailer($input)
    ...
    private function handleUpdateProductDetails($input)
    { //assuming that titles are unique
    ...
    { // only logged in normal users can leave reviews
        // ensure apiKey is NOT NULL
        if (empty($input['apikey']) || !isset($input['apikey'])) {
            http_response_code(400);
            $this->response("400 Bad Request", "error", "apikey is Missing");
    ...
    private function getUserDetails($input)
    {
        ...
        $query = "SELECT User_ID, Name, Surname, Email, Cell_No, User_Type FROM Users WHERE API_Key = ?";
